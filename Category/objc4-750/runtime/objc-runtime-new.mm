                                          ...
                                          header_info *catHeader)
{
    ...
    //列表类型为 locstamped_category_list_t(category_t  + header_info)
    //header_info：存储了实体在镜像中的加载和初始化状态，以及一些偏移量
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // Discover load methods  //根据地址来先找到 methods
    {
        mutex_locker_t lock2(runtimeLock);
        prepare_load_methods((const headerType *)mh); //准备加载Load方法
    }

    // Call +load methods (without runtimeLock - re-entrant)
    call_load_methods(); //加载load方法
}
...
static void schedule_class_load(Class cls)
{
    ...
    // Ensure superclass-first ordering  //确保父类 【先】
    schedule_class_load(cls->superclass);//把父类load先添加到数组中

    add_class_to_loadable_list(cls);//把自己的load方法添加到数组中
...
void prepare_load_methods(const headerType *mhdr)
{
    ...
    for (i = 0; i < count; i++) {
        schedule_class_load(remapClass(classlist[i])); //把类中的Load方法添加到数组中
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        add_category_to_loadable_list(cat);//把分类中的load方法添加到数组中
    }
}
