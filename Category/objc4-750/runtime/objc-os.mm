...
/***********************************************************************
 * _objc_init:
* Bootstrap initialization. Registers our image notifier with dyld.（引导初始化，通过【动态链接器、the dynamic link editor 的缩写】将所需要的【镜像】注册）
* Called by libSystem BEFORE library initialization time（由libSystem在库初始化时间之前调用）
**********************************************************************/
